...
class Validator:
    ...
    def final_evaluate(self, model,
        ...
        show_inp_fname = os.path.join(self.save_dir, 'tmp_show_inp.json')

        writeout = [{'show_inp': show_inp, 'truth': truth, 'pred': pred}
                    for show_inp, truth, pred in zip(show_inps, truths, preds)]
        fwrite(json.dumps(writeout, indent=4), show_inp_fname)

        writeout = ["{}\t{}\n".format(ix, pred) for ix, pred in
                    enumerate(preds)]
        fwrite(''.join(writeout), pred_fname)

        writeout = ["{}\t{}\n".format(ix, truth) for ix, truth
                    in enumerate(truths)]
        fwrite(''.join(writeout), truth_fname)

        # cmd = 'perl {} {} {}'.format(perl_fname, pred_fname, truth_fname)
        # stdout, stderr = shell(cmd, stdout=True)
